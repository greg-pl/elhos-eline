...
bool Cpx::findS(CxString *token, Cpx *result, const CpxDescr *aDef, void *aData, const char *name) {
					...
					if (Token::getAsInt(&name, " .[]", &idx)) {
						if (name[0] == ']' && name[1] == '.') {
							...
							if (idx < chInfo->itemCnt) {
								...
								return findS(token, result, chInfo->defs, dt, name);
							}
						}
...
bool Cpx::find(Cpx *result, const char *name) {
	...
	bool q = findS(token, result, mDef, mData, name);
	delete token;
	return q;
}

bool Cpx::findS(Cpx *result, const CpxDescr *aDef, void *aData, IdHist *idHist) {
			...
			if (aDef->ctype != cpxCHILD) {
				...
				if (n > 1) {
					...
					if (idTab >= 0 && idTab < n) {
						...
						return findS(result, chInfo->defs, dt, idHist);
					}
...
bool Cpx::find(Cpx *result, IdHist *idHist) {
	return findS(result, mDef, mData, idHist);
}
...
void Cpx::getAsTxt(const CpxDescr *def, const void *data, char *buf, int max) {
	...
	case cpxIP:
		ipaddr_ntoa_r((const ip_addr_t*) ptr, buf, max);
		break;
	case cpxTIME: {
...
void Cpx::show(const char *caption) {
	printf("%s: dscr=%p data=%p\n", caption, mDef, mData);
}
...
TStatus Cpx::InsertChanges(OutStream *strm, const uint8_t *data, int len) {
	...
	while (ptr < len) {
		...
		if (setm(key, dt, dt_sz)) {
			...
			strm->oMsgX(colCYAN, "Blad wstawiania cfg <%s>", ktxt);
			st = stCfgDataErr;
			break;
		}
		ptr += n;
