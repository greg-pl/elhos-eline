...
type
  ...
  TGetValEvent      = procedure(Sender: TObject; NrDanych: integer; NrProb : Cardinal ; var Val: real; var Exist: boolean) of object;
  TGetValAnEvent    = procedure(Sender: TObject; DtNr: integer; NrProb : Cardinal; var Val: double; var Exist: boolean) of object;
  TGetGroupAnEvent  = procedure(Sender: TObject; DtNr: integer; NrProb : Cardinal; ProbCnt : integer; Buf : TCxDataBuf) of object;
  TGetValDgEvent    = procedure(Sender: TObject; DtNr: integer; NrProb : Cardinal; var Val: boolean; var Exist: boolean) of object;
  TGetGroupDgEvent  = procedure(Sender: TObject; DtNr: integer; NrProb : Cardinal; ProbCnt : integer; Buf : TCxDgDataBuf) of object;
...
Const
  ...
  TabSeriesColor : array[0..SERIES_COLOR_TAB_LEN-1] of TColor =
    (clRed,clBlue,clGreen,clYellow,clBlack);

  TabPanelColor : array[0..PANELS_COLOR_TAB_LEN-1] of TColor =
    (TColor($DCF0D2),TColor($DCF0FA),TColor($E6FAFA),TColor($FFF0F0));
...
begin
  Result := Units;
  if Zakr<1        then Result := 'm'+Units;
  if    1000<=Zakr then Result := 'k'+Units;
  if 1000000<=Zakr then Result := 'M'+Units;
end;
...
begin
  s := '';
  if                      (Zakr<0.100)     then S:= Format('%5.5fm',[1000*V]); // 22.22 mV
  if (0.100<=Zakr)    and (Zakr<1)         then S:= Format('%5.4f',[V]); // 222.2 mV
  if (1<=Zakr)        and (Zakr<10)        then S:= Format('%5.3f',[V]);      // 2.222 V
  if (10<=Zakr)       and (Zakr<100)       then S:= Format('%5.2f',[V]);      // 22.22 V
  if (100<=Zakr)      and (Zakr<1000)      then S:= Format('%5.1f',[V]);      // 222.2 V
  if (1000<=Zakr)     and (Zakr<10000)     then S:= Format('%5.3fk',[V/1000]); // 2.222 kV
  if (10000<=Zakr)    and (Zakr<100000)    then S:= Format('%5.2fk',[V/1000]); // 22.22 kV
  if (100000<=Zakr)   and (Zakr<1000000)   then S:= Format('%5.1fk',[V/1000]); // 222.2 kV
  if (1000000<=Zakr)  and (Zakr<10000000)  then S:= Format('%5.3fM',[V/1000000]); // 2.222 MV
  if (10000000<=Zakr)                      then S:= Format('%5.2fM',[V/1000000]); // 22.22 MV
...
begin
  ...
  begin
    ...
    orHoriz: str :=(Wagon.Left-ScrPos.Left)/(ScrPos.Right-ScrPos.Left);
    orVert : str :=(Wagon.Top-ScrPos.Top)/(ScrPos.Bottom-ScrPos.Top);
    end;
...
begin
  ...
  orHoriz: begin
              ...
              begin
                Str := Fur+0.4*Fdur;
                if Str+Fdur>1.0 then Str:=1.0-Fdur;
                Result := true;
              end;
            end;
   orVert : begin
...
begin
  ...
  if du<5 then  du:=5;
  case FOrientation of
  orHoriz : Wagon := Rect(u,ScrPos.Top,u+du,ScrPos.Bottom);
  orVert  : Wagon := Rect(ScrPos.Left,u,ScrPos.Right,u+du);
  end;
end;
...
begin
  case FOrientation of
  orHoriz : ScrPos := Bounds(x,y,aLength,FThickness);
  orVert  : ScrPos := Bounds(x,y,FThickness,aLength);
  end;
  SetTimeVagonDim;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        Mooving:=True;
        case FOrientation of
        orHoriz: MoovingDD:=X-Wagon.Left;
        orVert:  MoovingDD:=Y-Wagon.Top;
        end;
      end
      else
      begin
...
begin
  ...
  if Result>=ProbCnt then Result := -1;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if aExist then
      begin
        if aVal>aMax then aMax := aVal;
        if aVal<aMin then aMin := aVal;
      end;
      inc(i);
    end;
    if aFirst then
...
begin
  ...
  if not Assigned(FWykres) then Exit;
  if not Assigned(FWykres.FOnGetAnValue) then Exit;
  if ProbCnt=0 then Exit;
  ...
  begin
    ...
    begin
      ...
      begin
        PtCntR := (NextX-x)/dx;
        PtCntL := trunc(PtCntR);
        if PtCntL<>PtCntR then inc(PtCntL);
        DoGroup := (PtCntL>2);
      end;
      ...
      begin
        ...
        begin
          ...
          if Exist then
          begin
            if V>MaxP then MaxP :=V;
            if V<MinP then MinP :=V;
            AnyExist := True;
          end;
          x := x+dx;
          inc(PtCnt);
...
begin
  ...
  begin
    ...
    begin
       ...
       if Exist then
       begin
         if (V>AMax) or First then AMax :=V;
         if (V<AMin) or First then AMin :=V;
         Suma := Suma+V;
         SumaKw := SumaKw + V*V;
         First := False;
         inc(NN);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if First or (Items[i].FMaxV>FMaxV) then FMaxV:=Items[i].FMaxV;
        if First or (Items[i].FMinV<FMinV) then FMinV:=Items[i].FMinV;
        First := false;
        FminMaxExist := true;
      end;
    end;  
...
var
  ...
  RR : TRect;
begin
  if V<DrawStart             then V:=DrawStart;
  if V>(DrawStart+DrawDelta) then V:=(DrawStart+DrawDelta);
  RR := GetWorkArea;
  w := RR.Bottom-RR.Top;
...
begin
   ...
   if Str<0 then Str := 0;
   if Str>1.0-Delt then Str:=1.0-Delt;
...
begin
  ...
  begin
    ...
    wmCursor,wmBox : Screen.Cursor := crTagMove;
    wmDelTag       : Screen.Cursor := crTagDel;
    end;
  end;
  if PtInRect(TextRect,Pt) and (FOwner.WorkMode in StabWorkMode) then
...
begin
  ...
  begin
    ...
    inc(sz.cy,2);
          ...
          if RR.Right-FTagX>sz.cx then  FAlign:=alRight
    else  if FTagX-RR.Left>sz.cx  then  FAlign:=alLeft
    else
    begin
...
begin
  ...
  begin
    ...
    case FAlign of
    alLeft : FOwner.FCanvas.LineTo(TextRect.Right,Pt.Y);
    alRight: FOwner.FCanvas.LineTo(TextRect.Left,Pt.Y);
    end;
  end;
end;
...
begin
  ...
  begin
    case Key of
      VK_LEFT:    KeyLeftFunc(Shift);
      VK_RIGHT:   KeyRightFunc(Shift);
      VK_INSERT:  KeyInsertFunc(Shift);
      VK_DELETE : KeyDeleteFunc(Shift);
      VK_RETURN:  if Assigned(Focused) then Focused.EditCaption;
      ord('C'):   KeyCopy(Shift);
    end;
  end;  
end;
...
begin
  ...
  if (FTimeWindowStart=Str) and (FTimeWindowDelta=Delt) then DoIt := false;
  ...
  begin
    ...
    Invalidate;
    if Assigned(FOnNewTimeZoom) then FOnNewTimeZoom(Self);
...
begin
  Result := LiczX(R);
  if Result = outLEFT  then Result := GDataArea.Left;
  if Result = outRight then Result := GDataArea.Right;
end;
...
begin
  ...
  if Result>ProbCnt then Result:=ProbCnt;
end;

function  TWykresEng.ProbkaToTime(nrPr: integer):double;
begin
       if nrPr<0 then result := outLEFT
  else if nrPr>ProbCnt then result := outRIGHT
  else Result := nrPr*DtPerProbka;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if ssShift in Shift then FMode := wmZoomVal;
          if ssCtrl in Shift then FMode := wmZoomTime;
        end;
...
function TWykresEng.MouseMove(Shift: TShiftState; X, Y: Integer): boolean;
  ...
  begin
    Result := false;
    if Screen.Cursor=crTagEdit then result := true;
    if Screen.Cursor=crTagDel  then result := true;
    if Screen.Cursor=crDel     then result := true;
    if Screen.Cursor=crSizeAll then result := true;
    if Screen.Cursor=crTagMove then result := true;
  end;
...
begin
  ...
  begin
    ...
    case FMode of
    wmZoomTime: Screen.Cursor:=crZoomT_In;
    wmZoomVal : Screen.Cursor:=crZoomV_In;
    wmBox     : Screen.Cursor:=crCrossBox;
    wmNewTag  : Screen.Cursor:=crNewTag;
    wmDelTag  : Screen.Cursor:=crDel;
    end;
  end;
...
begin
  ...
  begin
    if not(Zoomed) then
    begin
      if y<B1.ScrPos.Top    then y :=B1.ScrPos.Top;
      if y>B1.ScrPos.Bottom then y :=B1.ScrPos.Bottom;
    end;  
...
begin
  ...
  Cn.Rectangle(RR);
  Cn.MoveTo(RR.Left,RR.Top); Cn.LineTo(RR.Right,RR.Bottom);
  Cn.MoveTo(RR.Left,RR.Bottom); Cn.LineTo(RR.Right,RR.Top);
end;
...
begin
  ...
  if IsRectEmpty(FArea) then Exit;
...
begin
  ...
  begin
    case Key of
      VK_LEFT:  if not(ssShift in Shift) then
                  TimeShiftLeft
                else
                  TimeZoomOut;

      VK_RIGHT:  if not(ssShift in Shift) then
                   TimeShiftRight
                 else
                   TimeZoomIn;
    end;
  ...
  begin
    ...
    VK_INSERT :if not(FTagKey) then
               begin
                 FSplitPanelList.SetAllVisible;
                 LiczWymiary(rsmEqual);
                 Invalidate;
               end;
    VK_DELETE :if not(FTagKey) then
               begin
                 if FocusedPanel<>FSplitPanelList.GetLastPanel then
                   NewPanel:=FSplitPanelList.GiveNextPanel(FocusedPanel) as TChartPanel
                 else
    ...
    VK_PRIOR  :SetZoomed(true);
    VK_NEXT   :SetZoomed(False);
    VK_END    :begin
    ...
    VK_HOME   :  if not(ssShift in Shift) then
