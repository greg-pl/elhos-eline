...
type
  ...
  private
    ...
    function ReadStrPart(Var txt: string): TStatus;
  protected
    FNonBlkMode: boolean;
  ...
  public
    ...
    function Write_(txt: string): TStatus;
...
begin
  ...
  if Result = stOk then
  begin
    FSd := WinSock2.Socket(AF_INET, SOCK_DGRAM, IPPROTO_IP);
    if Sd = INVALID_SOCKET then
      Result := WSAGetLastError;
  end;
  EnableBrodcast(true);
  ...
  if Result = stOk then
  begin
    VPort := Ntohs(sockaddr_in(LAddr).sin_port);
    FSetAsync(true);
  end;
  Result := LoadLastErr(Result);
end;
...
begin
  ...
  begin
    ...
    begin
      SetLength(RecBuf, l);
      RecIp := String(inet_ntoa(sockaddr_in(RecAdr).sin_addr));
...
function TSimpTcp.Write_(txt: string): TStatus;
begin
  Result := stOk;
  inc(SendItemCnt);
  ...
  begin
    ...
    begin
      mNeagleaList.Add(txt);
      if GetTickCount - mNagleaFirstItemTick > cardinal(3 * mNagleTm) then
      begin
        SendNeagleList;
      end
      else
...
function TSimpTcp.ReadStrPart(Var txt: string): TStatus;
var
  l: u_long;
  Li: integer;
begin
...
begin
  ...
  RecIp := String(inet_ntoa(AddrIn.sin_addr));
