...
ONE_DESCRIPTOR CustomHID_CompatID_Descriptor = 
  {
    (uint8_t*)&USBD_CompatIDDesc,
    sizeof(USBD_CompatIDDesc)  
  };
ONE_DESCRIPTOR CustomHID_ExtProperties_Descriptor = 
  {
...
ONE_DESCRIPTOR String_Descriptor[5] =
  {
	...
	{(uint8_t*)CustomHID_StringWinUSB, CUSTOMHID_SIZ_STRING_WINUSB}
  };
...
RESULT CustomHID_Data_Setup(uint8_t RequestNo)
{
  ...
  else if(RequestNo == USB_REQ_GET_OS_FEATURE_DESCRIPTOR)
  {
   	if(pInformation->USBwIndexs.bw.bb0 == CustomHID_TYPE_OS_FEATURE_EXT_PROPERTIES)
		CopyRoutine = CustomHID_GetCompatIDDescriptor;
	else if(pInformation->USBwIndexs.bw.bb0 == CustomHID_TYPE_OS_FEATURE_EXT_COMPAT_ID)
		CopyRoutine = CustomHID_GetExtPropDescriptor;
...
uint8_t *CustomHID_GetStringDescriptor(uint16_t Length)
{
  ...
  if(wValue0 == 0xEE)
  {
    return Standard_GetDescriptorData(Length, &String_Descriptor[4]);
  }
  else if (wValue0 > 4)
  {
    return NULL;
...
/*******************************************************************************
* Function Name  : CustomHID_GetExtCompatDescriptor
* Description    : get the protocol value
* Input          : Length.
* Output         : None.
* Return         : address of the protocol value.
*******************************************************************************/
uint8_t *CustomHID_GetExtCompatDescriptor(uint16_t Length)
{
  if (Length == 0)
  {
    pInformation->Ctrl_Info.Usb_wLength = 1;
...
/*******************************************************************************
* Function Name  : CustomHID_GetExtPropDescriptor
* Description    : get the protocol value
* Input          : Length.
* Output         : None.
* Return         : address of the protocol value.
*******************************************************************************/
uint8_t *CustomHID_GetExtPropDescriptor(uint16_t Length)
{
	return Standard_GetDescriptorData(Length, &CustomHID_ExtProperties_Descriptor);
}
...
* Return         : address of the protocol value.
*******************************************************************************/
uint8_t *CustomHID_GetCompatIDDescriptor(uint16_t Length)
{
	return Standard_GetDescriptorData(Length, &CustomHID_CompatID_Descriptor);
}
