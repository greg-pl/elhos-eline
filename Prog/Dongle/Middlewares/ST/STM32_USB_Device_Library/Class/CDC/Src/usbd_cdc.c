...
__ALIGN_BEGIN static uint8_t USBD_CDC_DeviceQualifierDesc[USB_LEN_DEV_QUALIFIER_DESC] __ALIGN_END = {
		...
		0x00,  // USB Spec version 2.00
		0x02, 0x00,  // Device class
		0x00,  // device sub class
		0x00,  // device sub sub class
		0x40,  // max packet size
		0x01,  // no of configuration
		0x00,  // reserved
		};
...
static uint8_t USBD_CDC_Setup(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req) {
	...
	printf("SETUP:%u ",req->bmRequest & USB_REQ_TYPE_MASK);
	...
	case USB_REQ_TYPE_VENDOR:
		printf("VENDOR:%u,0x%02X ",req->bmRequest & 0x1F,req->bRequest);
		USBD_CtlError(pdev, req);
		ret = USBD_FAIL;
		break;
 ...
 */
static uint8_t* USBD_CDC_GetFSCfgDesc(uint16_t *length) {
	*length = WINUSB_CONFIG_DESC_SIZE;
