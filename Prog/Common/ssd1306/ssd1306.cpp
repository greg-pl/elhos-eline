...
SSD1306Dev::SSD1306Dev(uint8_t adr) {
	...
	memset(mBufferCpy, 0xFF, sizeof(mBufferCpy));
...
const ShellItem menuSSD1306[] = { //
		{ "SSD1306_init", "inicjuj SSD1306" }, //
		{ "SSD1306_test", "test screen" }, //
		{ NULL, NULL } };

const ShellItem* SSD1306Dev::getMenu() {
	return menuSSD1306;
}

bool SSD1306Dev::execFun(OutStream *strm, int idx, const char *cmd) {
	switch (idx) {
	case 0:
		mError = stOK;
		mState.Initialized = 0;
		Init();
		strm->oMsg("SSD1306 Init st=%s", getStatusStr(mError));
...
void SSD1306Dev::_WriteCmd(uint8_t cmd) {
	TStatus st = (TStatus)I2c1Bus::writeByte(mDevAdr, 0x00, cmd);
	if (st != stOK)
		mError = st;
}

void SSD1306Dev::_WriteData(uint8_t *buffer, size_t buff_size) {
	TStatus st = (TStatus)I2c1Bus::writeBytes(mDevAdr, 0x40, buff_size, buffer);
	if (st != stOK)
		mError = st;
}
...
void SSD1306Dev::updateScr(void) {
	if (I2c1Bus::openMutex(22, 100)) {

		for (int i = 0; i < 8; i++) {
			int idx = WIDTH * i;
			if (memcmp(&mBufferCpy[idx], &mBuffer[idx], WIDTH) != 0) {
				memcpy(&mBufferCpy[idx], &mBuffer[idx], WIDTH);
				_WriteCmd(0xB0 + i);
				_WriteCmd(0x00);
				_WriteCmd(0x10);
